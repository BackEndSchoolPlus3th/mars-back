...
class ServiceManager:
    ...
    def __init__(self, socat_port: int = 8081, sleep_duration: int = 3) -> None:
        ...
        self.services: Dict[str, int] = {
            'likelion_mars_1': 8082,
            'likelion_mars_2': 8083
        }
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'likelion_mars_2', self.services['likelion_mars_2']
        else:
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:8090 -e TZ=Asia/Seoul -v /dockerProjects/likelion_mars/volumes/gen:/gen --pull always ghcr.io/BackEndSchoolPlus3th/likelion_mars")
